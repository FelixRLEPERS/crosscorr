...
class AINarrator:
    ...
    def generate_story(self, significant_pairs: list[tuple[str, str]]) -> str:
        ...
        if not significant_pairs:
            story = (
                "(Тихий, таинственный голос): Хмм... Кажется, сегодня наш призрак "
                "был невероятно скрытным. Значимой связи мы найти не смогли! "
                "Это значит, что нам нужно собрать больше данных или попробовать "
                "другой метод. Не расстраивайтесь! Мы просто ещё не знаем, где искать. "
                "Давайте поиграем в 'угадай детектор' и подготовимся к завтрашнему дню!"
            )
            return story

        pairs_str = ", ".join(f"{a} и {b}" for a, b in significant_pairs)
        story_parts = [
            "Привет, юный исследователь!",
            "Похоже, наши кросс-корреляции дали нам невероятные новости!",
            f"Мы обнаружили, что детекторы {pairs_str} работают как команда! "
            f"Это значит, что их сигналы часто совпадают и подтверждают друг друга. "
            f"Это как два лучших друга, которые всегда говорят одно и то же!",
            "Это самая большая находка за неделю! Нам нужно детально изучить "
            "эти пары, чтобы понять научный смысл этой синхронности.",
        ]
